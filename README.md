#Barista
Baristaは架電APIです。システム障害時の連絡に必要な機能を持っています。  

* 一斉に架電する機能（全員から回答があるまで未回答の人に架電を繰り返す）
* 順番に架電する機能（誰か一人から回答があるまで繰り返す）
* 履歴を取得する機能

監視ツールと連携させることでオペレーション担当者を電話で叩き起こすことができます。(メールよりも効果ありますよね？)    
電話に応答した場合は回答した内容を、応答しなかった場合はその旨を取得できる機能も持っています。  
架電処理自体はtwilioを使って実現させています。Baristaはtwilioにこのような機能を付加したものです。
...
肯定的な回答を得られるまで繰り返し架電をするわけではありません。Paralellの場合、全員が否定的な回答をしても処理は完了となります。  
